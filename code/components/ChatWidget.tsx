...
export default function ChatWidget() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://localhost:5498/api/chat", {
        userMessage: input,
      });
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-16 h-16 rounded-full shadow-2xl transition-all duration-300 z-40 flex items-center justify-center ${
          isOpen
      ...
      {isOpen && (
        <div
          className={`fixed bottom-24 right-6 w-96 bg-white rounded-2xl shadow-2xl flex flex-col border overflow-hidden z-50 ${
            isMinimized ? "h-16" : "h-[600px]"
          }`}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-red-700 text-white p-5 flex justify-between items-center">
            <div>
              <h2 className="font-bold text-lg">NIET Assistant</h2>
              <p className="text-slate-300 text-xs">Powered by AI • Always here to help</p>
            </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 bg-slate-50 space-y-4 flex flex-col">
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="bg-white border px-4 py-3 rounded-2xl rounded-bl-none">
              ...
              <div className="border-t p-4 bg-white">
                <form onSubmit={handleSendMessage} className="flex gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 border rounded-full focus:outline-none focus:ring-2 focus:ring-red-600 text-sm"
                    disabled={isLoading}
                  />
                  <button
                    type="submit"
                    disabled={isLoading || !input.trim()}
                    className="bg-gradient-to-r from-red-600 to-red-500 text-white px-5 py-2.5 rounded-full shadow-md"
                  >
